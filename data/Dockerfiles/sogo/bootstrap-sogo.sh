...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  mysql --socket=/var/run/mysqld/mysqld.sock -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SET sql_log_bin = 0;DROP TRIGGER IF EXISTS sogo_update_password"
...
DELIMITER ;
EOF
    if [[ ! -z $(mysql --socket=/var/run/mysqld/mysqld.sock -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SET sql_log_bin = 1;SELECT 'OK' FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_NAME = 'sogo_update_password'") ]]; then
      TRIGGER_OK=OK
    else
