...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  php -c /usr/local/etc/php -f $EMAIL_PROJECT_ROOT/inc/init_db.inc.php
...
[[ ! -f $EMAIL_PROJECT_ROOT/css/build/0081-custom-mailcow.css ]] && echo '/* Autogenerated by mailcow */' > $EMAIL_PROJECT_ROOT/css/build/0081-custom-mailcow.css

# Fix permissions for global filters
chown -R 82:82 /global_sieve/*

# Fix permissions on twig cache folder
chown -R 82:82 $EMAIL_PROJECT_ROOT/templates/cache
# Clear cache
find $EMAIL_PROJECT_ROOT/templates/cache/* -not -name '.gitkeep' -delete
